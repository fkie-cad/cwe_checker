...
fn piecing_or_zero_extending() {
    ...
    assert!(check_defs_of_block(
        &block,
        vec!["zext_eax_to_rax: RAX:64 = IntZExt(0x0:32)"]
    ));
    ...
    assert!(check_defs_of_block(
        &block,
        vec!["zext_ah_to_rax: RAX:64 = IntZExt(0x0:8)"]
    ));
    ...
    assert!(check_defs_of_block(
        &block,
        vec![
            "eax_assign: RAX:64 = ((RAX:64)[4-7] Piece 0x0:32)",
    ...
    assert!(check_defs_of_block(
        &block,
        vec![
            "ah_assign: RAX:64 = (((RAX:64)[2-7] Piece 0x0:8) Piece (RAX:64)[0-0])",
    ...
    assert!(check_defs_of_block(
        &block,
        vec![
            "load_to_eax: loaded_value:32(temp) := Load from 0x0:64",
