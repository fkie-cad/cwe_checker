...
pub mod tests {
    ...
    fn test_eval_param_location() {
        ...
        let value =
            state.eval_param_location(&AbstractLocation::mock("r0:4", &[], 4), &global_memory);
        ...
        let value =
            state.eval_param_location(&AbstractLocation::mock("r0:4", &[8], 4), &global_memory);
        assert_eq!(
            value,
            DataDomain::from_target(
                AbstractIdentifier::new(
                    Tid::new("mock_fn"),
                    AbstractLocation::mock("r3:4", &[8], 4)
                ),
                bitvec!("0x0:4").into()
            )
        );
        ...
        // TODO: To test:
        // - register read
        // - pseudo read from nested register
        // - read from stack offset
        // - nested stack offset read
        todo!()
    }
}
