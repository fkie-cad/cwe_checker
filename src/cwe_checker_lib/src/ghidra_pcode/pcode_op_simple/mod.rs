...
impl Display for PcodeOp {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}: ({}", self.pcode_index, self.input0)?;
        if let Some(input1) = &self.input1 {
            write!(f, ", {}", input1)?;
        }
        if let Some(input2) = &self.input2 {
            debug_assert!(self.input1.is_some());
            write!(f, ", {}", input2)?;
        }
        write!(f, ")")?;
...
impl PcodeOp {
    ...
    pub fn operator(&self) -> &PcodeOperation {
        &self.pcode_mnemonic
    }

    pub fn input0(&self) -> &Varnode {
        &self.input0
    }
    ...
    pub fn has_implicit_load(&self) -> bool {
        if self.input0.is_in_ram() {
            return true;
        }
        if let Some(varnode) = &self.input1 {
            if varnode.is_in_ram() {
                return true;
            }
        }
