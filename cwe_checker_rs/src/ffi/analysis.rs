...
fn run_pointer_inference(program_jsonbuilder_val: ocaml::Value) -> Vec<CweWarning> {
...
caml!(rs_run_pointer_inference(program_jsonbuilder_val) {
    return failwith_on_panic( || {
        let cwe_warnings = run_pointer_inference(program_jsonbuilder_val);
        let cwe_warnings_json = serde_json::to_string(&cwe_warnings).unwrap();
        let ocaml_string = ocaml::Str::from(&cwe_warnings_json as &str);
...
fn run_pointer_inference_and_print_debug(program_jsonbuilder_val: ocaml::Value) {
    ...
    crate::analysis::pointer_inference::run(&project, true);
}
